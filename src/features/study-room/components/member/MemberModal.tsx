  ...
  return (
    <Modal {...props} onClose={onClose}>
      ...
      <Modal.Content className="flex flex-col items-center gap-4 p-4">
        ...
        <Typography.SubTitle1>{memberName}</Typography.SubTitle1>
        ...
        <div className="w-full border-t pt-3 text-center">
          <Typography.SubTitle1>출석률: {attendanceRate}%</Typography.SubTitle1>
        </div>
            ...
            <tbody>
              {attendanceRecords.map((record, index) => (
                <tr key={index} className="border-b">
                  <td className="p-2 text-center">{record.date}</td>

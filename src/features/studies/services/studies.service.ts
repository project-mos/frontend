...
export async function getStudy(id: string) {
  const response = await fetchData<GetStudyDetail>({
...
export async function getBenefits(studyId: string) {
  const response = await fetchData<GetStudyBenefits>({
